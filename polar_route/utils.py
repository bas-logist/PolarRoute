...
def memory_trace(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        stat = top_stats[0]
        logger.info("{} memory blocks: {.1f} KiB".format(stat.count, stat.size / 1024))
...
def pandas_dataframe_str(input_df):
    ...
    if (type(input_df) is dict) or (type(input_df) is pd.core.frame.DataFrame):
        ...
        try:
            ...
            raise Exception(
                "Unable to load '{}', please check path name".format(input_df)
            )
...
def to_chart_track_csv(route):
    ...
    header = (
        f"Route Name:,{route['properties']['from']}_{route['properties']['to']}\n"
    ...
    # Distance column
    cumulative_distance = (
        np.array(route["properties"]["distance"]) * 0.000539957
    )  # In nautical miles
...
def extract_geojson_routes(mesh):
    ...
    for route in routes:
        ...
        geojson_route = {"type": "FeatureCollection", "features": [route]}
