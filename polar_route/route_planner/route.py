...
class Route:
    ...
    def save(self, file_path):
        ...
        if file_path_strs[-1] in ["json", "geojson"]:
            ...
            raise ValueError(
                'Provide a path with a supported file extension: "json", "gpx"'
            )
    ...
    def waypoint_correction(self, cellbox, wp, cp, indx):
        ...
        m_lat = 111.386 * 1000
        x = (
            (cp.get_longitude() - wp.get_longitude())
            * m_long
            * np.cos(wp.get_latitude() * (np.pi / 180))
        )
        ...
        ssp = unit_speed(
            cellbox.agg_data["speed"][direction.index(case)],
            self.conf["unit_shipspeed"],
        )
        ...
        if "fuel" in self.conf["path_variables"]:
            self.segments[indx].set_fuel(
                cellbox.agg_data["fuel"][direction.index(case)] * traveltime
            )
            logger.debug(
                f"WP_correction >> fuel >> {cellbox.agg_data['fuel'][direction.index(case)] * traveltime}"
            )
        if "battery" in self.conf["path_variables"]:
            self.segments[indx].set_battery(
                cellbox.agg_data["battery"][direction.index(case)] * traveltime
            )
            logger.debug(
                f"WP_correction >> battery >> {cellbox.agg_data['battery'][direction.index(case)] * traveltime}"
            )
