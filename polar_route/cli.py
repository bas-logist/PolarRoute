...
def optimise_routes_cli():
    ...
    """
    args = get_args(
        "optimise_routes_output.route.json",
        config_arg=True,
        mesh_arg=True,
        waypoints_arg=True,
    )
    ...
    if args.dijkstra:
        ...
        info_dijkstra["paths"]["features"] = [
            dr.to_json(route_type="dijkstra") for dr in dijkstra_routes
        ]
    ...
    if args.path_gpx:
        logger.info("\tExtracting standalone path(s) to GPX file(s)")
        for route in smoothed_routes["features"]:
            from_wp = route["properties"]["from"].replace(" ", "_")
            to_wp = route["properties"]["to"].replace(" ", "_")
            route_output_str = (
                ".".join(output_file_strs[:-1]) + "_" + from_wp + to_wp + ".gpx"
            )
            gdf = gpd.GeoDataFrame.from_features([route])
            logger.info(f"Saving route to {route_output_str}")
...
def extract_routes_cli():
    ...
    logger.info(
        f"Extracting routes from: {args.mesh.name} with base output: {args.output}"
    )
    ...
    if output_file_strs[-1] in ["json", "geojson"]:
        ...
        for route in routes:
            ...
            to_wp = route["properties"]["to"].replace(" ", "_")
            route_output_str = (
                ".".join(output_file_strs[:-1]) + "_" + from_wp + to_wp + ".csv"
            )
...
def calculate_route_cli():
    ...
    """
    args = get_args(
        "calculated_route.json", config_arg=False, mesh_arg=True, waypoints_arg=True
    )
    logger.info("{} {}".format(inspect.stack()[0][3][:-4], version))

    logger.info(
        f"Calculating the cost of route {args.waypoints.name} from mesh {args.mesh.name}"
    )
    ...
    if calc_route is not None:
        max_time = convert_decimal_days(
            calc_route["features"][0]["properties"]["traveltime"][-1]
        )
        max_fuel = round(calc_route["features"][0]["properties"]["fuel"][-1], 2)

        logger.info(
            f"Calculated route has travel time: {max_time} and fuel cost: {max_fuel} tons"
        )
