...
class SDAWind(AbstractShip):
    ...
    def model_speed(self, cellbox):
        ...
        logger.debug(
            f"Calculating new speed for cellbox {cellbox.id} based on SDA resistance models"
        )
        ...
        if all(k in cellbox.agg_data for k in ("SIC", "thickness", "density")) and all(
            ...
            logger.debug("Not all ice data available, no ice resistance calculated")
                ...
                if cellbox.agg_data["wind resistance"][i] > 0:
                    if cellbox.agg_data["wind resistance"][i] < self.force_limit * 0.75:
                        cellbox.agg_data["speed"][i] = cellbox.agg_data["speed"][i] * (
                            1
                            - cellbox.agg_data["wind resistance"][i] / self.force_limit
                        )
                    else:
                        cellbox.agg_data["speed"][i] = (
                            cellbox.agg_data["speed"][i] * 0.25
                        )
                else:
                    cellbox.agg_data["speed"][i] = cellbox.agg_data["speed"][i] * (
                        1
                        - cellbox.agg_data["wind resistance"][i]
                        / (10 * self.force_limit)
                    )
    ...
    def model_fuel(self, cellbox):
        ...
        cellbox.agg_data["fuel"] = [
            fuel_eq(cellbox.agg_data["speed"][i], r)
    ...
    def model_resistance(self, cellbox):
        ...
        if "u10" in cellbox.agg_data and "v10" in cellbox.agg_data:
            ...
            cellbox.agg_data["resistance"] = [
                cellbox.agg_data["wind resistance"][i] + ice_resistance[i]
                for i in range(8)
            ]
        else:
    ...
    def ice_resistance(self, cellbox):
        ...
        speed = velocity * (5.0 / 18.0)  # assume km/h and convert to m/s
    ...
    def invert_resistance(self, cellbox):
        ...
        new_speed = vms * (18.0 / 5.0)  # convert from m/s to km/h
    ...
    def wave_resistance(self, w_height):
        ...
        c_block = self.vessel_params.get(
            "c_block", 0.75
        )  # ratio of underwater volume to cuboid
        length = self.vessel_params["Length"]

        wave_res = (
            0.64 * rho_w * c_block * (w_height**2) * (beam**2)
        ) / length  # Kreitner, valid up to ~2m wave height
...
def fuel_eq(speed, resistance):
    ...
    fuel = (
        ...
        - 0.0029601 * speed
        + 0.25290433
        + 7.75218178e-11 * resistance**2
        + 6.48113363e-06 * resistance
    ) * 24.0
    return fuel
...
def wind_resistance(v_speed, w_speed, rel_ang):
    ...
    wind_res = 0.5 * rho * (w_speed**2) * a * c_wind(rel_ang) - 0.5 * rho * (
        v_speed**2
    ) * a * c_wind(0)
...
def calc_wind(cellbox):
    ...
    for i, head in enumerate(heads):
        ...
        wind_res[i] = wind_resistance(
            cellbox.agg_data["speed"][i] * 0.277778,
            rel_wind_speed[i],
            rel_wind_angle[i],
        )  # assume km/h and convert to m/s
