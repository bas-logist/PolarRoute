...
def test_land(sda_vessel, base_cellbox):
    ...
    base_cellbox.agg_data = {'elevation': -100.}
    assert sda_vessel.land(base_cellbox) == False


def test_extreme_ice(sda_vessel, base_cellbox):
    """Test extreme ice detection for 100% concentration."""
    base_cellbox.agg_data = {'SIC': 100.}
    assert sda_vessel.extreme_ice(base_cellbox) == True
...
@pytest.mark.parametrize(
    ...
    [
        # Open water case
        ({'speed': 26.5, 'SIC': 0., 'thickness': 0., 'density': 0.},
         {'speed': [26.5]*8, 'SIC': 0., 'thickness': 0., 'density': 0., 'ice resistance': 0.}),
        # Ice case
        ({'speed': 26.5, 'SIC': 60., 'thickness': 1., 'density': 980.},
         {'speed': [7.842665122593933]*8, 'SIC': 60., 'thickness': 1., 'density': 980., 'ice resistance': 96634.5}),
    ],
    ids=['open_water', 'ice']
)
...
@pytest.mark.parametrize(
    ...
    [
        # Open water
        ({'speed': [26.5]*8, 'SIC': 0., 'thickness': 0., 'density': 0., 'ice resistance': 0.},
         {'speed': [26.5]*8, 'SIC': 0., 'thickness': 0., 'density': 0., 'ice resistance': 0., 'resistance': [0.]*8}),
        # Ice
        ({'speed': [7.842665122593933]*8, 'SIC': 60., 'thickness': 1., 'density': 980., 'ice resistance': 96634.5},
         {'speed': [7.842665122593933]*8, 'SIC': 60., 'thickness': 1., 'density': 980., 'ice resistance': 96634.5, 'resistance': [96634.5]*8}),
    ],
    ids=['open_water', 'ice']
)
...
@pytest.mark.parametrize(
    ...
    [
        (0., 0., 0., 0., 0.),
        (5.56, 60., 1., 980., 64543.75549708632),
    ],
    ids=['zero', 'positive']
)
def test_ice_resistance(sda_vessel, base_cellbox, speed, sic, thickness, density, expected):
    """Test ice resistance calculation."""
    base_cellbox.agg_data = {'speed': speed, 'SIC': sic, 'thickness': thickness, 'density': density}
...
@pytest.mark.parametrize(
    ...
    [
        (26.5, 0., 0., 0., 26.5),
        (26.5, 60., 1., 980., 7.842665122593933),
    ],
    ids=['zero_resistance', 'positive_resistance']
)
def test_invert_resistance(sda_vessel, base_cellbox, speed, sic, thickness, density, expected):
...
@pytest.mark.parametrize(
    ...
    [
        (0., 0., 6.06970392),  # Hotel load
        (26.5, 0., 27.3186897),  # Open water
...
@pytest.mark.parametrize(
    "angle, expected",
    [(0., 0.94), (np.pi/4., 0.595)],
    ids=['zero', 'interpolated']
)
...
@pytest.mark.parametrize(
    ...
    [
        (0., 0., 0., 0.),
        (10., 10., 0., 0.),  # Equal opposite
        (10., 20., 0., 129543.75),
        (10., 20., np.pi, -105656.25),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ([0.]*8, 0., 0., (0., 0.)),
        ([10.]*8, 0., 10., (7.22222, np.pi)),
        ([10.]*8, 10., 0., (10.378634, 1.299849)),
    ],
    ids=['zero', 'north', 'east']
)
def test_wind_mag_dir(base_cellbox, speed, u10, v10, expected):
    ...
    result = wind_mag_dir(base_cellbox, 0.)
